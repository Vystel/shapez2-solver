...
export class ShapeSolverController {
    ...
    processState(currentState, combo, op, outputs, extraData, nextLevel, visited, currentDepth) {
        const reuseShapes = document.getElementById('reuse-shapes-checkbox')?.checked;

        let newAvailable;
        if (reuseShapes) {
            // Keep the used shapes (reuse mode)
            newAvailable = [...currentState.availableShapes];
        } else {
            // Default: remove the used shapes
            newAvailable = currentState.availableShapes.filter(s => !combo.includes(s));
        }
        ...
        if (op === operations.painter || op === operations.crystalGenerator) {
